...
CLIP_MODEL_PATHS = {
    ...
    'vit_base_patch16_224': '/NS/robustness_2/work/vnanda/invariances_in_reps/deep-learning-base/checkpoints/clip/ViT-B-16.pt',
    'resnet50': '/NS/robustness_2/work/vnanda/invariances_in_reps/deep-learning-base/checkpoints/clip/RN50.pt',
    'resnet101': '/NS/robustness_2/work/vnanda/invariances_in_reps/deep-learning-base/checkpoints/clip/RN101.pt'
}
...
def create_model(model_name: str, 
    ...
    if dataset_name.endswith('cifar10') and not use_timm_for_cifar:
        ...
        assert pretrained, f'For CLIP models, pretrained must be True!'
        ## make sure checkpoints are saved as needed by CLIP 
        ## (https://github.com/openai/CLIP/blob/main/clip/clip.py)
        if checkpoint_path == '':
            warnings.warn(f'No CLIP checkpoint given, using default CLIP model: {CLIP_MODEL_PATHS[model_name]}')
        model, _ = clip.load(CLIP_MODEL_PATHS[model_name], device='cpu')
        ## CLIP VIT models are not symbolically traceable; wrap
        model.visual = make_symbolic(model.visual)
        if not multimodal_clip:
            ## only take the visual component
            model = model.visual 
        ...
        if checkpoint_path != '' and checkpoint_path.split('/')[-1] != CLIP_MODEL_PATHS[model_name].split('/')[-1]:
